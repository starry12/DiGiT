...
struct BAM_Feature_Store {
    ...
    // Phase 9B generalized mixed-I/O geometry. The serialized semantic hash is
    // held on the host; numeric fields are checked against page_cache_d_t.
    bool mixed_io_enabled = false;
    unsigned long long int* d_mixed_io_stats = nullptr;
    uint32_t mixed_feature_row_bytes = 0;
    uint32_t mixed_min_transfer_bytes = 0;
    uint32_t mixed_group_size = 0;
    uint32_t mixed_subrow_count = 0;
    uint64_t mixed_full_mask = 0;
    bool mixed_split_cache_entries = false;
    uint32_t cache_entry_size = 0;
    std::string mixed_geometry_sha256;

    // Phase 8C CPU feature path: 0 = mapped-host direct gather, 1 = compacted
    // pinned-host staging followed by one asynchronous H2D payload transfer.
    uint32_t cpu_feature_path = 0;
    uint32_t cpu_staging_threads = 8;
    uint64_t cpu_staging_reserve_rows = 131072;
    std::vector<uint32_t> cpu_page_offsets;
    std::vector<uint64_t> staging_source_rows;
    std::vector<uint64_t> staging_output_positions;
    int64_t* staging_host_indices = nullptr;
    uint8_t* staging_host_payload = nullptr;
    uint8_t* staging_device_payload = nullptr;
    uint64_t staging_index_capacity = 0;
    uint64_t staging_payload_capacity = 0;
    cudaStream_t staging_ssd_stream = nullptr;
    cudaStream_t staging_copy_stream = nullptr;
    cudaEvent_t staging_h2d_start = nullptr;
    cudaEvent_t staging_h2d_end = nullptr;
    cudaEvent_t staging_scatter_end = nullptr;
    bool staging_runtime_initialized = false;
    uint64_t staging_batches = 0;
    uint64_t staging_cpu_rows = 0;
    uint64_t staging_payload_bytes = 0;
    uint64_t staging_index_bytes = 0;
    uint64_t staging_reallocations = 0;
    uint64_t staging_index_copy_ns = 0;
    uint64_t staging_host_gather_ns = 0;
    uint64_t staging_h2d_ns = 0;
    uint64_t staging_scatter_ns = 0;
     ...
     */
    void init_controllers(GIDS_Controllers GIDS_ctrl, uint32_t ps, uint64_t r_off, uint64_t cache_size, uint64_t num_ele,
                          uint64_t num_ssd, uint32_t replacement_policy = 0,
                          uint32_t cache_entry_bytes = 0);
    ...
    void read_feature(uint64_t tensor_ptr, uint64_t index_ptr, int64_t num_index,
                      int dim, int cache_dim, uint64_t key_off,
                      uint64_t group_flags_ptr = 0);

    void get_io_stat(uint64_t tensor_ptr);
    std::vector<uint64_t> get_gpu_cache_stats();
    std::vector<uint64_t> get_feature_access_stats();
    void set_mixed_io(bool enabled);
    void set_mixed_io_geometry(bool enabled, uint32_t feature_row_bytes,
                               uint32_t minimum_transfer_bytes,
                               uint32_t group_size, uint32_t subrow_count,
                               uint64_t full_valid_mask,
                               const std::string& geometry_sha256);
    std::vector<uint64_t> get_mixed_io_stats();
    std::vector<uint64_t> get_mixed_io_geometry();
    std::string get_mixed_io_geometry_hash();
    void set_device_io_stats(bool enabled);
    std::vector<uint64_t> get_device_io_stats();
    void reset_device_io_stats();
    void begin_useful_io_region();
    std::vector<uint64_t> get_useful_io_stats();
    bool useful_io_enabled = false;
    uint64_t useful_region_id = 0;
    void set_cpu_feature_path(uint32_t path, uint64_t reserve_rows = 131072,
                              uint32_t gather_threads = 8);
    std::vector<uint64_t> get_cpu_staging_stats();
    /**
    ...
    unsigned long long int get_cpu_access_count_total();
    unsigned long long int get_gpu_access_count_total();
    ...
    void ensure_cpu_staging_runtime(uint64_t index_rows,
                                    uint64_t payload_rows);
    void read_feature_cpu_staged(uint64_t tensor_ptr, uint64_t index_ptr,
                                 uint64_t group_flags_ptr,
