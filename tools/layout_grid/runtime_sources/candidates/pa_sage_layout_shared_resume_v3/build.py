...
import shutil
import resource
from candidates.pa_sage_layout_sweep_v1.common import *
...
class Context:
    ...
    def check_resources(self):
        ...
        require(shutil.disk_usage(existing_parent(self.output.parent)).free>=b['filesystem_bytes_with_scratch_and_reserve']-b['payload_bytes_max'],'Insufficient filesystem space upper bound')
...
def build_layout(ctx):
    ...
    from .common import verify as verify_shared
    execution=verify_shared()
    with exclusive():
        result=_build_layout(ctx)
        require(verify_shared()==execution,'Shared preparation code changed')
        result['shared_candidate_sha256']=execution;write(ctx.output/'build_receipt.json',result)
        return result

def _build_layout(ctx):
    import numpy as np
    from candidates.pa_sage_layout_sweep_v1 import metadata
    ...
    try:
        for name,desc in ctx.inputs.items():
            ...
            expected=desc.get('observed_identity')
            if expected is not None:require(before==expected,'Input file identity mismatch: '+name)
            strict=ctx.point['id']=='g2_r20'
            if strict:require(sha(path)==desc['sha256'],'Input contents changed')
            bindings[name]=dict(path=str(path),sha256=desc['sha256'],identity=before,content_hash_checked=strict)
        ptr,idx=map(array,ctx.csc_paths());hot=array(ctx.inputs['hot_nodes']['path'])
        bindings['features']=dict(ctx.inputs['features'],role='unused_logical_feature_reference_not_loaded')
        ...
        stages_to_run=[('metadata',metadata.make_metadata)]
        if ctx.point['id']=='g2_r20':stages_to_run.append(('metadata_validation',metadata.validate_metadata))
        ...
        if ctx.point['id']!='g2_r20':
            write(ctx.output/'final/validation.json',dict(passed=None,status='skipped_by_user',
                  expanded_adjacency_multiset_exact=None,storage_inverse_exact=None,group_padding_checked=None,
                  hot_excluded=None,descriptor_sha256=sha(ctx.output/'final/descriptor.json')))
        for desc in bindings.values():
            if 'identity' in desc:require(identity(desc['path'])==desc['identity'],'Input changed during build')
        require(code_bindings()==code,'Build code changed during preparation')
        m=read(ctx.output/'final/descriptor.json')
        m.setdefault('metadata',{})['shared_test_pool']=dict(feature_payload_materialized=False,feature_semantics='physical_row_proxy')
        write(ctx.output/'final/bundle/manifest.json',m)
        g=m['grouping'];rows=m['feature']['num_storage_rows']
        ...
        report=dict(passed=True,point=ctx.point,fixture=ctx.fixture,manifest_sha256=sha(ctx.output/'final/bundle/manifest.json'),
                    ...
                    expanded_directed_adjacency_exact=True if ctx.point['id']=='g2_r20' else None,full_graph_semantics_checked=ctx.point['id']=='g2_r20',filesystem_features_bit_exact=False,feature_payload_materialized=False,metadata_only=True,
