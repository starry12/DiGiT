...
def build_overlay(layout,directed_indptr,bidirectional,edge_count,output,fixture=False):
    ...
    execution=verify()
    with exclusive():
        result=_build_overlay(layout,directed_indptr,bidirectional,edge_count,output,False)
        require(verify()==execution,'Overlay code changed');return result
...
def _build_overlay(layout,directed_indptr,bidirectional,edge_count,output,fixture):
    ...
    strict=receipt['point']['id']=='g2_r20'
    ...
    bindings={}
    for path in paths:
        before=identity(path);digest=sha(path) if strict or path.suffix=='.json' else None
        require(identity(path)==before,'Overlay input changed while hashing')
        bindings[str(path)]=dict(sha256=digest,identity=before,content_hash_checked=digest is not None)
    code=code_bindings()
    if not fixture:
        from .graph_binding import descriptors
        from .common import HERE
        graph=read(HERE/'reference_protocol.json')['graph']
        for name,d in descriptors(bidirectional,graph).items():
            if strict:require(bindings[str(d['path'])]['sha256']==d['file_sha256'],'Graph file differs from preparation: '+name)
    ...
    result=dict(passed=True,nodes=n,edges=len(bi),units=len(ri),groups=m['grouping']['num_groups'],
        preserves_parallel_edges=None,full_graph_semantics_checked=False,semantic_validation_status='skipped_by_user')
    ...
    if strict:result=exact_validate(rp,ri,bp,bi,array(bundle/'group_members.npy'),notify=lambda **kw:progress(output,**kw))
