...
def report_check(r,arm,smoke,binding,folder):
    ...
    for i,e in enumerate(r['epochs']):
        ...
        if arm=='gids' and not smoke:require(all(e['selected_source_bridge'].values()),'Selected-source trajectory bridge failed')
...
def pair_check(reports,smoke):
    ...
    return dict(passed=True,training_speedup=a['training_seconds']/b['training_seconds'],
                ...
                scope='short native gate only' if smoke else 'single seed0, selected bidirectional PA/SAGE, fixed inherited g2 layout')
