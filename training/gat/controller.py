"""Fresh GAT output, paired native smoke gate, then both complete seed-0 workers."""
...
from training.gat.inputs import input_binding
from training.gat.worker import check_inputs
...
def worker_command(job,output):
    cmd=[sys.executable,'-u','-m','training.gat.worker','--arm',job['arm'],
...
def execute(mode,output,gpu):
    ...
    state=dict(schema='digit-pa-gat-run-v1',mode=mode,dataset='PA',model='gat',seed=0,gpu=gpu,passed=False,complete=False,
    ...
    try:
        ...
        launch=dict(schema='digit-pa-gat-launch-v1',mode=mode,dataset='PA',model='gat',seed=0,gpu=gpu,candidate_sha256=execution,
    ...
    if mode=='representative':
        ...
        from training.gat.summarize import load_formal,emit
