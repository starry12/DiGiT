"""Configuration-driven GAT loop; measurement and evaluation follow the extracted runtime."""
import math
from runner import *
from training.gat.evaluation import EvaluationSession
from training.gat.common import cfg,HERE,source_config
...
def run_training(arm,value,loader,bundle,graph,labels,degrees,output,data,protocol,smoke=False,features=None):
    ...
    return dict(model_name="gat",model_config=cfg()["model_config"],model_semantics=cfg()["model_semantics"],model_parameter_count=sum(p.numel() for p in model.parameters()),metadata_setup_seconds=metadata_setup_seconds,optimizer_effective={k:v for k,v in optimizer.param_groups[0].items() if k!="params"},evaluation_lifecycle=evaluation_summary,passed=True,arm=arm,seed=value,smoke=smoke,epochs=epochs,updates=sum(item["updates"] for item in epochs),
