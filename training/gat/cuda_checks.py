"""Tiny actual CUDA sampler plus GAT updates; no raw SSD or PA-sized allocation."""
import tempfile
import numpy as np
...
def run_checks():
    ...
    from training.gat.tests import math_check
    ...
    with tempfile.TemporaryDirectory(prefix='digit-gat-cuda-') as td:
        ...
        for mode in ('gpu','gpu_i32_uva_eid64'):
            ...
            for step in range(3):
                ...
                optimizer.zero_grad(set_to_none=True);loss.backward();require(all(torch.isfinite(q.grad).all() for q in model.parameters()),'Nonfinite GAT gradient');optimizer.step()
        ...
        require(records[0]==records[1],'GAT training differs between int64 CUDA and mixed-width UVA sampler')
        require(sum(s['groups'] for s in records[1])>0,'GAT fixture did not exercise grouped sampling')
        graph._graph.unpin_memory_()
    torch.cuda.synchronize()
    return dict(passed=True,manual_gat_math=math_result,updates_per_backend=3,fanouts=p['fanouts'],g2_grouped_sampling=True,
