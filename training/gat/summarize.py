"""Revalidate this frozen GAT native pair, then emit phase metrics with qualifications."""
import argparse,csv,json,math,sys
from pathlib import Path
from training.gat.common import *
from runtime.io.accounting import summarize as io_summary
from training.gat.validation import pair_check
...
def metrics(arm,r):
    ...
    return dict(dataset='PA',model='gat',seed=0,repeat=0,arm=arm,epochs=len(r['epochs']),updates=r['updates'],
...
def comparison(reports,provenance,source):
    ...
    return dict(schema='digit-submission-results-v2',passed=True,complete=True,training_accuracy_io_passed=True,strict_resource_acceptance=strict,
        ...
        protocol_sha256=sha(P),model_config=cfg()['model_config'],model_semantics=cfg()['model_semantics'],optimizer=cfg()['optimizer'],
        ...
        limitations=['Only reconstructed bidirectional PA/GAT, seed0; inherited g2 layout and no BFS.',
                     'Optimizer lr=0.01/weight_decay=0.001 and dropout=0.2 are historical GAT reconstruction choices, not specified in the paper or tuned on this test.',
                     'Four heads with 128 hidden features per head (512 concatenated) follow the historical implementation; the paper does not specify head count or width interpretation.',
...
def load_formal(folder,allow_smoke=False):
    from training.gat.controller import plan,worker_command
    from training.gat.review import review_worker
    folder=Path(folder).resolve();state=read(folder/'status.json');launch=read(folder/'launch.json');binding=read(folder/'inputs.json')
    require(state['schema']=='digit-pa-gat-run-v1' and state['passed'] and state['complete'],'Submission run incomplete')
    require(launch['schema']=='digit-pa-gat-launch-v1' and launch['mode']==state['mode'],'Wrong launch')
    require(launch['candidate_sha256']==state['candidate_sha256']==sha(HERE/'manifest.json'),'Wrong submission version')
    require(launch['worker_candidate_sha256']==state['worker_candidate_sha256']==binding['candidate_sha256']==sha(ROOT/'training/gat/manifest.json'),'Wrong worker version')
    ...
    require(launch['dataset']==state['dataset']=='PA' and launch['model']==state['model']=='gat' and launch['seed']==state['seed']==0,'Wrong dataset/model/seed')
    ...
    for job,w in zip(jobs,state['workers']):
        ...
        from training.gat.controller import arm_evidence
    ...
    value=comparison(phases['full'],'submission_v4_pa_gat_fresh_native_pair',dict(directory=str(folder),launch_sha256=sha(folder/'launch.json')))
...
def emit(value,output):
    ...
    lines=['# PA / GAT measured results','', 'Provenance: `'+value['provenance']+'`.', '',
...
def main():
    ...
    output=output_path(a.output);verify();value=load_formal(a.input);emit(value,output)
