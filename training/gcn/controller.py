...
from training.gcn.inputs import input_binding
from training.gcn.worker import check_inputs
...
def worker_command(job,output):
    cmd=[sys.executable,'-u','-m','training.gcn.worker','--arm',job['arm'],
...
def execute(mode,output,gpu):
    ...
    state=dict(schema='digit-pa-gcn-run-v1',mode=mode,dataset='PA',model='gcn',seed=0,gpu=gpu,passed=False,complete=False,
    ...
    try:
        ...
        launch=dict(schema='digit-pa-gcn-launch-v1',mode=mode,dataset='PA',model='gcn',seed=0,gpu=gpu,candidate_sha256=execution,
    ...
    if mode=='representative':
        ...
        from training.gcn.summarize import load_formal,emit
