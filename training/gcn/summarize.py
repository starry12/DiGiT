...
from training.gcn.validation import pair_check
...
def metrics(arm,r):
    ...
    return dict(dataset='PA',model='gcn',seed=0,repeat=0,arm=arm,epochs=len(r['epochs']),updates=r['updates'],
...
def comparison(reports,provenance,source):
    ...
    return dict(schema='digit-submission-results-v2',passed=True,complete=True,training_accuracy_io_passed=True,strict_resource_acceptance=strict,
        ...
        limitations=['Only reconstructed bidirectional PA/GCN, seed0; inherited g2 layout and no BFS.',
                     'Optimizer lr=0.01/weight_decay=0.001 and dropout=0.2 are historical GCN reconstruction choices, not specified in the paper or tuned on this test.',
...
def load_formal(folder,allow_smoke=False):
    from training.gcn.controller import plan,worker_command
    from training.gcn.review import review_worker
    folder=Path(folder).resolve();state=read(folder/'status.json');launch=read(folder/'launch.json');binding=read(folder/'inputs.json')
    require(state['schema']=='digit-pa-gcn-run-v1' and state['passed'] and state['complete'],'Submission run incomplete')
    require(launch['schema']=='digit-pa-gcn-launch-v1' and launch['mode']==state['mode'],'Wrong launch')
    require(launch['candidate_sha256']==state['candidate_sha256']==sha(HERE/'manifest.json'),'Wrong submission version')
    require(launch['worker_candidate_sha256']==state['worker_candidate_sha256']==binding['candidate_sha256']==sha(ROOT/'training/gcn/manifest.json'),'Wrong worker version')
    from evaluation.gcn.common import verify as verify_entry
    ...
    require(launch['dataset']==state['dataset']=='PA' and launch['model']==state['model']=='gcn' and launch['seed']==state['seed']==0,'Wrong dataset/model/seed')
    ...
    for job,w in zip(jobs,state['workers']):
        ...
        from training.gcn.controller import arm_evidence
    ...
    value=comparison(phases['full'],'submission_v3_pa_gcn_fresh_native_pair',dict(directory=str(folder),launch_sha256=sha(folder/'launch.json')))
...
def emit(value,output):
    ...
    lines=['# PA / GCN measured results','', 'Provenance: `'+value['provenance']+'`.', '',
