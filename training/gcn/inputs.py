...
def compatibility():
    ...
    require(p['model_config']==read(ROOT/'configs/papers/contract.json')['models']['gcn'],'Wrong historical GCN contract')
...
def input_binding(output,execution,require_preflight=True):
    ...
    if require_preflight:
        ...
        require(status['passed'] and status['complete'] and status['candidate_sha256']==execution,'GCN preflight incomplete or stale')
        require(status['entry_manifest_sha256']==sha(ROOT/'evaluation/gcn/manifest.json'),'Preflight entry version differs')
        require(status['model_checks']['passed'] and status['cuda_checks']['passed'] and status['live_admission']['passed'],'GCN model/resource checks failed')
    ...
    value.update(candidate_sha256=execution,protocol_sha256=sha(P),parent_data_binding_sha256=parent_sha,
                 parent_data_candidate_sha256=sha(PARENT/'manifest.json'),model='gcn',compatibility=compatibility())
    if require_preflight:
        for name in ('status.json','inputs.json','model_checks.json','cuda_checks.json'):
