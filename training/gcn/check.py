"""GCN model/CUDA admission and complete input hashes, before any native SSD run."""
import argparse,os,time,subprocess,sys
from training.gcn.common import *
from training.gcn.inputs import input_binding,compatibility
...
def checked_child(module,output):
    ...
    # Both check modules print one final JSON object after diagnostics.
...
def check(output,gpu):
    ...
    start=time.time();state=dict(schema='digit-pa-gcn-preflight-v1',passed=False,complete=False,candidate_sha256=candidate,
    ...
    save()
    try:
        model_checks=checked_child('training.gcn.tests',output);write(output/'model_checks.json',model_checks)
        save(stage='cuda_model_sampler_checks')
        cuda_checks=checked_child('training.gcn.cuda_checks',output);write(output/'cuda_checks.json',cuda_checks)
        ...
        plan=check_live(estimate());require(plan['passed'],'Live GCN GPU/host budget failed')
        require(plan['free_bytes']>plan['total_bytes']-2**30,'Selected GPU already in use')
        write(output/'admission.json',plan)
        save(stage='hashing_unchanged_pa_inputs',live_admission=plan,model_checks=model_checks,cuda_checks=cuda_checks)
        ...
        save(passed=True,complete=True,stage='complete',compatibility=compatibility(),
             evidence_sha256={n:sha(output/n) for n in ('inputs.json','parent_data_inputs.json','model_checks.json','cuda_checks.json','admission.json','trace_contracts.json','tests.log','cuda_checks.log')},
             seconds=time.time()-start,finished_unix=time.time(),
             scope='GCN math/CUDA tests, selected PA graph/source input hashes, trace payload hashes/contracts and live resource admission. Existing g2 SSD payload reused; full g2 artifact verification also runs inside native DiGiT workers. No raw SSD access or new performance evidence.')
