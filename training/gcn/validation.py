...
def report_check(r,arm,smoke,binding,folder):
    ...
    require(r['model_name']=='gcn' and r['model_config']==p['model_config'] and r['model_semantics']==p['model_semantics'] and r['model_parameter_count']==p['model_parameter_count'],'Wrong GCN model')
    require(r['optimizer_effective']==p['optimizer']['kwargs'],'Wrong GCN optimizer')
...
def pair_check(reports,smoke):
    ...
    require(all(r['model_name']=='gcn' and r['model_config']==cfg()['model_config'] and r['optimizer_effective']==cfg()['optimizer']['kwargs'] for r in reports.values()),'Unpaired GCN protocol')
    ...
    return dict(passed=True,training_speedup=a['training_seconds']/b['training_seconds'],
                ...
                scope='short native gate only' if smoke else 'single seed0, reconstructed bidirectional PA/GCN, fixed inherited g2 layout')
