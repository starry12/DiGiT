...
A fresh online installation into a separate prefix passed on 2026-09-23, including `pip check`, the locked package/import checks and the three-model CPU example. Native compilation and module imports also passed after correcting the identity utility build to use C++11 and the BaM/CUDA include paths; see the [build receipt](../provenance/native_build_validation.json). Compiler warnings remain in the archived build log. After restoring the omitted `GIDS.breakdown` dependency, the rebuilt copy passed PA/SAGE, GCN and GAT GPU/SSD preflight and paired smoke; see the [native acceptance receipt](../reference/rebuilt_native_smoke.json). The five native binaries were reused unchanged from the successful build. This acceptance used existing prepared read-only inputs; a complete fresh download-to-layout-to-SSD pipeline remains unverified. Full experiment evidence still belongs to the separately identified server or historical runs.
