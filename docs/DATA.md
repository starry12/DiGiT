...
The selected graph preserves original multiedges, adds reverse edges and exactly one self-loop per node: 3,342,431,700 edges. Both systems use the same graph, random training orders and sampled evaluation traces. DiGiT uses the prepared g2 layout. The GPU cache is 4 GiB and the static CPU cache contains 11,105,992 rows.
