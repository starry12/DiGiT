...
int ctrl_chrdev_create(struct ctrl* ctrl, dev_t first, const struct file_operations* fops)
{
    ...
    ctrl->chrdev = chrdev;
    struct pci_dev* dev = ctrl->pdev;

    printk(KERN_INFO "Character device /dev/%s created (%d.%d) Adding controller device: %02x:%02x.%1x\n",
            ctrl->name, MAJOR(ctrl->rdev), MINOR(ctrl->rdev), dev->bus->number, PCI_SLOT(dev->devfn), PCI_FUNC(dev->devfn));
