...
__global__ void read_feature_kernel(array_d_t<T> *dr, T *out_tensor_ptr, int64_t *index_ptr, int dim, int64_t num_idx, int cache_dim, uint64_t key_off) {
    ...
    if (idx_idx < num_idx) {  // 确保当前的全局索引不超出总索引范围
        ...
        for (; tid < dim; tid += 32) {
            ...
            if (idx_idx == 0){
                //printf("Temp value at idx_idx %lld, tid %d: %f\n", idx_idx, tid, temp);
            }
            //printf("Temp value at idx_idx %lld, tid %d: %f\n", idx_idx, tid, temp);
...
// 读取特征并写入文件
void save_to_file(float *out_tensor_ptr, int64_t node_count, int dim) {
    // 打开文件用于写入
    std::ofstream out_file("10_5_5_feature_one_batch_test_5.txt", std::ios::out);
    if (!out_file.is_open()) {
    ...
    for (int64_t i = 0; i < node_count; ++i) {
        for (int j = 0; j < dim; ++j) {
            out_file << out_tensor_ptr[i * dim + j] << " ";
        }
        out_file << std::endl;
    }
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        for (size_t i = 0; i < (int)586; i++) {
            ...
            //wq
            //node_sequence[0] = 72665;
            //node_sequence[1] = 3;

            
            //for (uint64_t i = 0; i < node_count; ++i) {
                //std::cout << "node_sequence[" << i << "] = " << node_sequence[i] << std::endl;
                //std::cout << "node_sequence[" << i << "] = " << std::dec << node_sequence[i] << std::endl;
            //}

            //std::cout << "node_sequence_0 = " << node_sequence[0] << std::endl;
            ...
            //wq
            //FEATURE_TYPE *h_out_tensor_ptr;
            //size_t out_tensor_size = node_count * FEATURE_DIM * sizeof(FEATURE_TYPE);
            //h_out_tensor_ptr = new FEATURE_TYPE[node_count * FEATURE_DIM];
            ...
            //wq
            //cudaMemcpy(h_out_tensor_ptr, d_data, out_tensor_size, cudaMemcpyDeviceToHost);
            // 将数据写入文件
            //save_to_file(h_out_tensor_ptr, node_count, FEATURE_DIM);
            ...
            //wq
            //delete[] h_out_tensor_ptr;
            if (i % 1000 == 0){
                printf("FIN: Batch %lu done! \n", i);
            }
            
        }
        printf("all_node_count: %lu\n", all_node_count);
        double ssd_bandwidth = (((double)total_data) / (total_time / 1000000)) / (1024ULL * 1024ULL * 1024ULL);

        a->print_reset_stats();
        double bandwidth = (((double)total_data) / (total_time / 1000000)) / (1024ULL * 1024ULL * 1024ULL) * 4.0;
        ...
        // bandwidth << std::endl;
        std::cout << std::dec << "Elapsed Time(s): " << total_time / (1000 * 1000) << "\tTotal Data Size (GBytes): " << total_data / 1024.0 / 1024.0 / 1024.0 * 4.0
                  << std::endl;
