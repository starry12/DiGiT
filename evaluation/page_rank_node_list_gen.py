...
def compute_pagerank(g, DAMP, K, N):
    ...
    # reversed_g = dgl.reverse(g, copy_ndata=True)
    
    # # Run regular PageRank on the reversed graph
    # reversed_g.ndata['pv'] = torch.ones(N) / N
    # out_degrees = reversed_g.out_degrees(reversed_g.nodes()).type(torch.float32)
    # out_degrees = torch.where(out_degrees > 0, out_degrees, torch.ones_like(out_degrees))
    
    # for k in range(K):
    #     # Standard PageRank iteration on the reversed graph
    #     reversed_g.ndata['pv'] = reversed_g.ndata['pv'] / out_degrees
        
    #     reversed_g.update_all(
    #         message_func=fn.copy_u('pv', 'm'),
    #         reduce_func=fn.sum('m', 'pv')
    #     )
        
    #     reversed_g.ndata['pv'] = (1 - DAMP) / N + DAMP * reversed_g.ndata['pv']
    # # Copy results back to original graph if needed
    # g.ndata['pv'] = reversed_g.ndata['pv']
    return g.ndata['pv']
