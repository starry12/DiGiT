    ...
    , deviceId(cudaDevice)
{
    ...
    // Create admin queue memory
    // 分配大小为控制器页面大小的3倍（ctrl->page_size * 3），通常用于命令队列和完成队列
    // 分配的是主机内存
    // 分配一段与控制器相关联的DMA（直接内存访问）区域
    aq_mem = createDma(ctrl, ctrl->page_size * 3);

    initializeController(*this, ns_id);

    // 将控制器的内存映射到CUDA主机内存
    // ctrl->mm_ptr: 控制器的内存映射区域（BAR0寄存器）
    // NVM_CTRL_MEM_MINSIZE: 映射区域的最小大小
    // cudaHostRegisterIoMemory: 注册内存为IO映射区域，供CUDA访问
