...
// uint32_t n_ctrls = 1;
const char* const sam_ctrls_paths[] = {"/dev/libnvm1", "/dev/libnvm4", "/dev/libnvm9", "/dev/libnvm2", "/dev/libnvm3",
                                       "/dev/libnvm5", "/dev/libnvm6", "/dev/libnvm7", "/dev/libnvm8"};
const char* const intel_ctrls_paths[] = {"/dev/libinvm1", "/dev/libinvm4", "/dev/libinvm9", "/dev/libinvm2", "/dev/libinvm3",
...
// "/dev/libnvm7", "/dev/libnvm8", "/dev/libnvm9", "/dev/libnvm10", "/dev/libnvm11", "/dev/libnvm12", "/dev/libnvm13", "/dev/libnvm14", "/dev/libnvm15",
// "/dev/libnvm16", "/dev/libnvm17", "/dev/libnvm18", "/dev/libnvm19", "/dev/libnvm20", "/dev/libnvm21", "/dev/libnvm22", "/dev/libnvm23",
...
__device__ void read_data(page_cache_t* pc, QueuePair* qp, const uint64_t starting_lba, const uint64_t n_blocks, const unsigned long long pc_entry) {
    ...
    //printf("tid: %llu\tstart_lba: %llu\tn_blocks: %llu\tprp1: %p\n", (unsigned long long) threadIdx.x, (unsigned long long) starting_lba, (unsigned long long)
n_blocks, (void*) prp1); nvm_cmd_data_ptr(&cmd, prp1, prp2); nvm_cmd_rw_blks(&cmd, starting_lba, n_blocks); uint16_t sq_pos = sq_enqueue(&qp->sq, &cmd);
...
__global__ 
void sequential_access_kernel(Controller** ctrls, page_cache_d_t* pc, uint32_t req_size, uint32_t n_reqs,
                                                                   unsigned long long* req_count, uint32_t num_ctrls, uint64_t reqs_per_thread,
...
__global__
    void
    random_access_kernel(Controller** ctrls, page_cache_d_t* pc, uint32_t* req_size_array, uint32_t n_reqs, unsigned long long* req_count, uint32_t num_ctrls,
                         uint64_t* assignment, uint64_t reqs_per_thread, uint32_t access_type, uint8_t* access_type_assignment) {
    // printf("in threads\n");
    // n_reqs：请求的总数
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n_reqs) return;

    // 根据线程索引获取本次请求大小（字节）
    uint32_t req_size = req_size_array[tid];
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        //     ctrls[i] = new Controller(settings.ssdtype == 0 ? sam_ctrls_paths[i] : intel_ctrls_paths[i], settings.nvmNamespace, settings.cudaDevice,
        //                               settings.queueDepth, settings.numQueues);
...
#define TYPE uint64_t
        ...
        /*         uint64_t t_size = n_blocks * sizeof(TYPE);
                ...
                range_t<uint64_t> h_range((uint64_t)0, (uint64_t)n_elems, (uint64_t)0, (uint64_t)(t_size/page_size), (uint64_t)0, (uint64_t)page_size, &h_pc,
                settings.cudaDevice); range_t<uint64_t>* d_range = (range_t<uint64_t>*) h_range.d_range_ptr;
        ...
        // 访问的地址队列 //表示每个线程所分配的请求位置
        uint64_t* assignment;
        uint64_t* d_assignment;
        if (settings.random) {
            assignment = (uint64_t*)malloc(n_threads * sizeof(uint64_t));  // 在主机内存中分配一个大小为 n_threads * sizeof(uint64_t) 的数组 assignment
            for (size_t i = 0; i < n_threads; i++)                         // 范围在 0 到 n_blocks-1 之间
                assignment[i] = rand() % (n_blocks);                       // 生成一个随机数，表示每个线程分配到的块的位置
        ...
        if (settings.accessType == 2) {
            ...
            for (size_t i = 0; i < n_threads; i++) access_assignment[i] = (((rand() % 100) + 1) <= settings.ratio) ? NVM_IO_READ : NVM_IO_WRITE;
        ...
        // 新增：为每个线程分配请求大小（4KB 或 8KB）
        uint32_t* req_sizes = (uint32_t*)malloc(n_threads * sizeof(uint32_t));
        size_t cutoff = (size_t)(n_threads * 0.6);  // 前 40% 线程发 8 KiB，剩下 60% 发 4 KiB
        size_t half = n_threads / 2;

        //size_t  x = 17825792, y = 3932160;
        //assert(x + y == n_threads);
        //size_t x = 16777216, y = 4194304;
        for (size_t i = 0; i < n_threads; i++) {
            //req_sizes[i] = (i < cutoff) ? 4096 : 16384;
            req_sizes[i] = (i < half) ? 4096 : 4096 ;
            //req_sizes[i] = (i < x) ? 16384 : 16384;
        }

        uint32_t* d_req_sizes;
        cudaMalloc(&d_req_sizes, n_threads * sizeof(uint32_t));
        cudaMemcpy(d_req_sizes, req_sizes, n_threads * sizeof(uint32_t), cudaMemcpyHostToDevice);

        std::cout << "atlaunch kernel\n";
        if (settings.random)
            random_access_kernel<<<g_size, b_size>>>(h_pc.pdt.d_ctrls, d_pc, d_req_sizes, n_threads, d_req_count, settings.n_ctrls, d_assignment,
                                                     settings.numReqs, settings.accessType, d_access_assignment);
        else
            sequential_access_kernel<<<g_size, b_size>>>(h_pc.pdt.d_ctrls, d_pc, page_size, n_threads, d_req_count, settings.n_ctrls, settings.numReqs,
                                                         settings.accessType, d_access_assignment);
        Event after;
        ...
        // 总 I/O 次数
        uint64_t ios = n_threads * settings.numReqs;

        // 累加每个线程实际读取的字节数
        uint64_t data = 0;
        for (uint64_t i = 0; i < n_threads; ++i) {
            data += (uint64_t)req_sizes[i] * settings.numReqs;
        }
        ...
        std::cout << std::dec << "Elapsed Time(s): " << elapsed / (1000 * 1000) << "\tTotal Data Size (GBytes): " << data / 1024.0 / 1024.0 / 1024.0 << std::endl;
        ...
        for (size_t i = 0; i < settings.n_ctrls; i++) delete ctrls[i];
...
__global__
    void
    random_access_kernel(Controller** ctrls, page_cache_d_t* pc, uint32_t req_size, uint32_t n_reqs, unsigned long long* req_count, uint32_t num_ctrls,
                         uint64_t* assignment, uint64_t reqs_per_thread, uint32_t access_type, uint8_t* access_type_assignment) {
    // printf("in threads\n");
    // n_reqs：请求的总数
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
#define TYPE uint64_t
        ...
        std::cout << "atlaunch kernel\n";
        if (settings.random)
            random_access_kernel<<<g_size, b_size>>>(h_pc.pdt.d_ctrls, d_pc, page_size, n_threads, d_req_count, settings.n_ctrls, d_assignment,
                                                     settings.numReqs, settings.accessType, d_access_assignment);
        else
            sequential_access_kernel<<<g_size, b_size>>>(h_pc.pdt.d_ctrls, d_pc, page_size, n_threads, d_req_count, settings.n_ctrls, settings.numReqs,
                                                         settings.accessType, d_access_assignment);
